--[[
    ...
    v3.2: Static ID DB only (.h sourced) | Full match apply
]]

_G.YargiEngine = _G.YargiEngine or {}
_G.YargiEngine.Loaded = false
_G.YargiEngine.Version = "3.2"
...
function Yargi.loadStaticSkinDatabase()
    ...
    local function mergeList(key, target)
        if not (data[key] and type(data[key]) == "table") then return end
        local tkey = target or key
        local cur = _G.OutfitSkins[tkey]
        if type(cur) ~= "table" then
            _G.OutfitSkins[tkey] = data[key]
            return
        end
        local exists = {}
        for _, v in ipairs(cur) do exists[tonumber(v) or v] = true end
        for _, v in ipairs(data[key]) do
            local id = tonumber(v) or v
            if id ~= 0 and not exists[id] then
                cur[#cur + 1] = id
                exists[id] = true
            end
        end
        _G.OutfitSkins[tkey] = cur
    end

    mergeList("Suit", "Suit")
    mergeList("Bag", "Bag")
    mergeList("Helmet", "Helmet")
    mergeList("Gloves", "Gloves")
    mergeList("Pants", "Pants")
    mergeList("Parachute", "Parachute")
    mergeList("Glider", "Glider")

    if data.LobbyTheme then _G.LobbyThemeSkins = data.LobbyTheme end
    if data.skinIdMappings then
        for baseId, list in pairs(data.skinIdMappings) do
    ...
    if data.VehskinIdMappings then
        for vType, list in pairs(data.VehskinIdMappings) do
            _G.VehskinIdMappings[tonumber(vType) or vType] = list
        end
    end

    Yargi.refreshOutfitMaps()
    Yargi.staticSkinLoaded = true
    print("[YARGI] Embedded Skin DB yuklendi")
    return true
end
...
function Yargi.ensureAllSkinsRegistered()
    ...
    local count = 0
    local regOutfitKeys = {"Bag", "Helmet", "Gloves", "Pants", "Suit"}
...
_G.YargiEngine.Start = function()
    print("[YARGI ENGINE v3.2] Static IDs | no runtime dump | full match apply")
end

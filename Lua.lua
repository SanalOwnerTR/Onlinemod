...
_G.YARGI_ALIVE = true
print("[YARGI] chunk load")

_G.YargiEngine = _G.YargiEngine or {}
_G.YargiEngine.Loaded = false
_G.YargiEngine.Version = "3.6"

local Yargi = {}


-- Skin DB: YargiData.lua (Clismio 2. dosya) veya loadfile
...
local function buildConfigPaths()
    local paths = {}
    local base = resolveDataPath()
    paths[#paths + 1] = base .. "/yargi_config.ini"
    paths[#paths + 1] = base .. "/YargiConfig.ini"
    paths[#paths + 1] = base .. "/config.ini"
    for _, pkg in ipairs(PACKAGE_LIST) do
        paths[#paths + 1] = "/storage/emulated/0/Android/data/" .. pkg .. "/files/yargi_config.ini"
        paths[#paths + 1] = "/storage/emulated/0/Android/data/" .. pkg .. "/files/UE4Game/ShadowTrackerExtra/ShadowTrackerExtra/Saved/Config/Android/yargi_config.ini"
    end
    return paths
end

function resolveKillCounterPath()
    return resolveDataPath() .. "/yargi_kills.lua"
end

local CONFIG_PATHS = nil

function Yargi.getConfigPaths()
    if not CONFIG_PATHS then CONFIG_PATHS = buildConfigPaths() end
    return CONFIG_PATHS
end

-- Minimal fallback if YargiData.lua not loaded
...
_G.ActiveKillCounterPath = resolveDataPath() .. "/yargi_kills.lua"
pcall(function() _G.ActiveKillCounterPath = resolveKillCounterPath() end)
...
function Yargi.readConfigContent()
    for _, path in ipairs(Yargi.getConfigPaths()) do
...
function _G.equip_character_avatar(uCharacter)
    ...
    if _G.PantsSkin ~= 0 and S.PantsEquipemtSlot then Yargi.ensureSlot(applyData, S.PantsEquipemtSlot) end
    ...
    for i = 0, applyData:Num() - 1 do
        ...
        if S.PantsEquipemtSlot then
            Yargi.setSlotSkin(avatarComp, applyData, i, _G.PantsSkin, S.PantsEquipemtSlot, "pants")
        end
    end
end
    ...
    if Yargi._SKIN_DB_RAW then
        ...
        if fn then
            ...
            if ok and type(res) == "table" then
                Yargi._parsedSkinDb = res
                return res
            end
            print("[YARGI] embed parse err: " .. tostring(res))
        else
            print("[YARGI] embed load err: " .. tostring(err))
        end
    end
    return nil
end
...
function Yargi.loadStaticSkinDatabase()
    ...
    local outfitKeys = {
        "Suit", "Bag", "Helmet", "Gloves", "Pants", "Parachute", "Glider",
        "Pet", "PetDress", "Head", "Hair", "Hat", "Face", "Armor",
        "HandEffect", "LastKill", "FinalKill", "VehicleEffect",
    }
    for _, key in ipairs(outfitKeys) do
        if data[key] and type(data[key]) == "table" then
            _G.OutfitSkins[key] = data[key]
        end
    end

    if data.LobbyTheme then _G.LobbyThemeSkins = data.LobbyTheme end
    if data.skinIdMappings then _G.skinIdMappings = data.skinIdMappings end
    if data.GrenadeSkins then
        for baseId, list in pairs(data.GrenadeSkins) do
            _G.skinIdMappings[tonumber(baseId) or baseId] = list
        end
    end
...
function Yargi.ensureAllSkinsRegistered()
    if Yargi.allSkinsRegistered then return end
    if not Yargi.staticSkinLoaded then Yargi.loadStaticSkinDatabase() end
    local count = 0
    local regOutfitKeys = {
        "Suit", "Bag", "Helmet", "Gloves", "Pants", "Parachute", "Glider",
        "Pet", "PetDress", "Head", "Hair", "Hat", "Face", "Armor",
    ...
    for _, key in ipairs(regOutfitKeys) do
        local list = _G.OutfitSkins[key] or {}
    ...
    if _G.LobbyThemeSkins then
        for _, resID in ipairs(_G.LobbyThemeSkins) do
            Yargi.registerFakeResID(resID)
            count = count + 1
        end
    end
...
function Yargi.matchRealtimeTick()
    ...
    pcall(function()
        ...
        if _G.IsPtrValid(uChar) then
            ...
            _G.UpdateWeapon_BackPack_Appearance(pc, true)
        end
        _G.GameAvatarHandlervehicles()
        _G.GameAvatarHandlerkillcounter()
    end)
...
function Yargi.inferWardrobeFields(resID)
    resID = tonumber(resID)
    if not resID then return nil end
    if resID >= 1501001000 and resID < 1501016000 then
        return { mainTabType = 1, itemSubType = ENUM_ITEM_SUBTYPE and ENUM_ITEM_SUBTYPE.Upgrade_Backpack or 501 }
    elseif resID >= 1502001000 and resID < 1502016000 then
        return { mainTabType = 1, itemSubType = 502 }
    elseif (resID >= 402000 and resID < 403000) or (resID >= 452000 and resID < 453000) then
        return { mainTabType = 1, itemSubType = ENUM_ITEM_SUBTYPE and ENUM_ITEM_SUBTYPE.Gloves or 402 }
    elseif resID >= 1410000 and resID < 1420000 then
        return { mainTabType = 1, itemSubType = ENUM_ITEM_SUBTYPE and ENUM_ITEM_SUBTYPE.Pants_Slot or 405 }
    elseif resID >= 1400000 and resID < 1415000 then
        return { mainTabType = 1, itemSubType = 403 }
    elseif resID >= 703000 and resID < 704000 then
        return { mainTabType = 4, itemSubType = 703 }
    elseif resID >= 4151000 and resID < 4152000 then
        return { mainTabType = 4, itemSubType = 4151 }
    end
    return nil
end
...
function Yargi.ensureItemFields(item)
    ...
    pcall(function()
        ...
        if cfg then
            ...
            local inferred = Yargi.inferWardrobeFields(item.resID)
            if inferred then
                item.mainTabType = inferred.mainTabType
                item.itemSubType = inferred.itemSubType
                item.itemType = ENUM_ITEM_TYPE and ENUM_ITEM_TYPE.Extra or 0
                item.itemQuality = 7
            end
        end
...
function Yargi.categorizeResId(resID)
    resID = tonumber(resID)
    if not resID then return end
    if resID >= 1501001000 and resID < 1501016000 then
        Yargi.appendOutfitId("Bag", resID)
    elseif resID >= 1502001000 and resID < 1502016000 then
        Yargi.appendOutfitId("Helmet", resID)
    elseif (resID >= 402000 and resID < 403000) or (resID >= 452000 and resID < 453000) then
        Yargi.appendOutfitId("Gloves", resID)
    elseif resID >= 1410000 and resID < 1420000 then
        Yargi.appendOutfitId("Pants", resID)
    elseif resID >= 1400000 and resID < 1415000 then
        Yargi.appendOutfitId("Suit", resID)
    elseif resID >= 703000 and resID < 704000 then
        Yargi.appendOutfitId("Parachute", resID)
    elseif resID >= 4151000 and resID < 4152000 then
        Yargi.appendOutfitId("Glider", resID)
    end
    Yargi.registerFakeResID(resID)
    Yargi.refreshOutfitMaps()
end
...
function Yargi.wardrobeMaintainTick()
    ...
    Yargi.dumpSkinLoaded = true
    local now = os.clock()
    if not Yargi.depotInjectReady or (now - (Yargi.lastDepotInject or 0)) > 30 then
        Yargi.injectFakeItemsToDepot()
        Yargi.lastDepotInject = now
    end
    if (now - (Yargi.lastArmoryRebuild or 0)) > 40 then
        Yargi.injectArmorySkinList(true)
    end
end
...
local function YargiStartTimers()
    ...
    _G.Mytimer_ticker.AddTimerLoop(0, function() pcall(Yargi.wardrobeMaintainTick) end, -1, 12.0)
...
local function YargiScheduleBootstrap()
    ...
    if ok and TXtime_ticker and TXtime_ticker.AddTimerOnce then
        _G.Mytimer_ticker = TXtime_ticker
        TXtime_ticker.AddTimerOnce(0.5, function() pcall(Yargi.bootstrap) end)
        TXtime_ticker.AddTimerOnce(3, function()
            if not _G.YargiEngine.Loaded then pcall(Yargi.bootstrap) end
        end)
        TXtime_ticker.AddTimerOnce(8, function()
            if not _G.YargiEngine.Loaded then pcall(Yargi.bootstrap) end
        end)
        print("[YARGI] bootstrap zamanlayici")
        return true
    end
    return false
end

if not YargiScheduleBootstrap() then
    print("[YARGI] zamanlayici yok, direkt bootstrap")
    pcall(Yargi.bootstrap)
end

_G.YargiEngine.Start = function()
    pcall(Yargi.bootstrap)
    print("[YARGI ENGINE v3.6] Lua.lua + YargiData.lua | Clismio")
end
